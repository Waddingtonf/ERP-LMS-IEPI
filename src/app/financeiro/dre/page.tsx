        ...
        <div className="space-y-8">
            <div className="flex items-start justify-between">
                <div>
                    ...
                    <p className="text-slate-500 mt-1 text-sm">Período: {dre.periodo}</p>
                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 bg-gradient-to-r from-violet-700 to-indigo-700 text-white">
                    ...
                    <div className="text-white/70 text-sm">{dre.periodo} · Valores em BRL</div>
                </div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {dre.linhas.map(linha => {
                                ...
                                const variacao: number | null = null; // period comparison not available
                                const pctReceita = linha.percentualReceita.toFixed(1);
                                ...
                                const isLucro = cfg.bold && !isNegative && linha.valor > 0;
                                return (
                                    <tr key={linha.tipo} className={`${cfg.separator ? 'border-t-2 border-slate-300' : ''} ${cfg.bold ? 'bg-slate-50/80' : 'hover:bg-slate-50/40'} transition-colors`}>
                                        ...
                                        <td className={`px-4 py-3 text-right font-${cfg.bold ? 'bold' : 'medium'} ${isLucro ? (linha.valor >= 0 ? 'text-emerald-700 text-base' : 'text-rose-700 text-base') : isNegative ? 'text-rose-700' : 'text-slate-800'}`}>
