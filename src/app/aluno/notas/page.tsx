...
const SITUACAO_CONFIG = {
    ...
    'Recuperacao':  { label: 'RecuperaÃ§Ã£o',    color: 'bg-amber-100 text-amber-700 border-amber-200',       icon: AlertCircle, iconColor: 'text-amber-500' },
...
export default async function AlunoNotasPage() {
    ...
    const mediaGeral = mediasValidas.length > 0 ? (mediasValidas.reduce((a, b) => a + b, 0) / mediasValidas.length).toFixed(1) : 'â€”';

    return (
        <div className="space-y-8">
            {/* Header */}
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Boletim AcadÃªmico</h1>
                <p className="text-slate-500 mt-1 text-sm">Acompanhe suas notas e situaÃ§Ã£o por disciplina em cada turma.</p>
            </div>

            {/* KPI Strip */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                    { label: 'MÃ©dia Geral', value: mediaGeral, icon: TrendingUp, color: 'text-violet-600', bg: 'bg-violet-50' },
                    { label: 'AprovaÃ§Ãµes', value: aprovadas, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                    ...
                    <div key={h.turmaId} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="bg-gradient-to-r from-violet-700 to-indigo-700 px-6 py-4 flex items-center justify-between">
                            ...
                            <div className="text-right">
                                <div className="text-white/70 text-xs">FrequÃªncia</div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/70">
                                        ...
                                        <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">MÃ©dia</th>
                                        <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">SituaÃ§Ã£o</th>
                                    </tr>
                                </thead>
                                    ...
                                    {h.notas.map(nota => {
                                        ...
                                        return (
                                            <tr key={nota.id} className="hover:bg-slate-50/60 transition-colors">
                                                ...
                                                <td className={`px-4 py-4 text-center ${notaColor(nota.av1)}`}>{nota.av1 ?? 'â€”'}</td>
                                                <td className={`px-4 py-4 text-center ${notaColor(nota.av2)}`}>{nota.av2 ?? 'â€”'}</td>
                                                <td className={`px-4 py-4 text-center ${notaColor(nota.trabalho)}`}>{nota.trabalho ?? 'â€”'}</td>
                                                <td className={`px-4 py-4 text-center text-base ${notaColor(nota.media)}`}>{nota.media ?? 'â€”'}</td>
