    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                <p className="text-slate-500 mt-1 text-sm">Visão gerencial consolidada — {dre.periodo}</p>
            </div>

            {/* KPI Cards */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {indicadores.map(ind => {
                    const positivo = ind.tendencia === 'up';
                    const Icon = ICON_MAP[ind.chave] ?? ICON_MAP['default'];
                    return (
                        <div key={ind.chave} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                            ...
                            <div className="text-xl font-bold text-slate-900">
                                {ind.formato === 'moeda' ? fmtMoeda(Number(ind.valor)) : ind.formato === 'percentual' ? `${Number(ind.valor).toFixed(1)}%` : String(ind.valor)}
                            </div>
                            <div className="text-xs text-slate-500 mt-1">{ind.label}</div>
                        </div>
                    );
                })}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="px-6 py-4 space-y-2">
                    <div className="flex items-center justify-between py-2 border-b border-slate-50">
                        ...
                        <span className="text-sm font-bold text-emerald-600">{fmtMoeda(dre.receitaBruta)}</span>
                    </div>
                    <div className="flex items-center justify-between py-2 border-b border-slate-50">
                        <span className="text-sm font-semibold text-slate-700">Despesas Operacionais</span>
                        <span className="text-sm font-bold text-rose-600">{fmtMoeda(dre.despesasOperacionais)}</span>
                    </div>
                    <div className={`flex items-center justify-between py-3 rounded-xl px-3 ${dre.lucroLiquido >= 0 ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                        <span className="font-bold text-slate-800">Lucro Líquido</span>
                        <span className={`text-lg font-bold ${dre.lucroLiquido >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>{fmtMoeda(dre.lucroLiquido)}</span>
                    </div>
                </div>
            </div>
